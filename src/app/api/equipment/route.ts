...
export const GET = withSafeHandler(async () => {
  ...
  const { data, error } = await supabase.from('equipment').select('*').eq('organization_id', orgId).order('name')
...
export const POST = withSafeHandler(async (req: Request) => {
  ...
  const orgId = await getOrgId(supabase, user.id)
  if (!orgId) return NextResponse.json({ error: 'No organization' }, { status: 400 })
  const { data, error } = await supabase.from('equipment').insert({ ...parsed.data, organization_id: orgId }).select().single()
  if (error) return NextResponse.json({ error: error.message }, { status: 500 })
