...
export const GET = withSafeHandler(async () => {
  ...
  const { data, error } = await supabase.from('pipeline_deals').select('*').eq('organization_id', orgId).order('created_at', { ascending: false })
...
export const POST = withSafeHandler(async (req: Request) => {
  ...
  createNotification({
    ...
    title:          `New deal: ${data.name}`,
    description:    data.contact_name ? `Contact: ${data.contact_name}` : undefined,
    link:           '/pipeline',
  }).catch(() => {})
