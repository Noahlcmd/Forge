...
export const PATCH = withSafeHandler(async (req: Request, { params }: { params: { id: string } }) => {
  ...
  const { data, error } = await supabase.from('pipeline_deals').update(parsed.data).eq('id', params.id).eq('organization_id', orgId).select().single()
  if (error) return NextResponse.json({ error: error.message }, { status: 500 })
  if (parsed.data.stage) {
    createNotification({
      organizationId: orgId,
      title:          `Deal moved to "${parsed.data.stage}"`,
      description:    `Deal: ${data.name}`,
      link:           '/pipeline',
    }).catch(() => {})
  }
...
export const DELETE = withSafeHandler(async (_: Request, { params }: { params: { id: string } }) => {
  ...
  const orgId = await getOrgId(supabase, user.id)
  if (!orgId) return NextResponse.json({ error: 'No organization' }, { status: 400 })
  const { error } = await supabase.from('pipeline_deals').delete().eq('id', params.id).eq('organization_id', orgId)
  if (error) return NextResponse.json({ error: error.message }, { status: 500 })
