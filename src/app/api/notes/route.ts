...
export const GET = withSafeHandler(async () => {
  ...
  const { data, error } = await supabase.from('notes').select('*').eq('organization_id', orgId).order('updated_at', { ascending: false })
...
export const POST = withSafeHandler(async (req: Request) => {
  ...
  const orgId = await getOrgId(supabase, user.id)
  if (!orgId) return NextResponse.json({ error: 'No organization' }, { status: 400 })
  const { data, error } = await supabase.from('notes').insert({ ...parsed.data, organization_id: orgId, created_by: user.id }).select().single()
  if (error) return NextResponse.json({ error: error.message }, { status: 500 })
